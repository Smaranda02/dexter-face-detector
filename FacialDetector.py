...
class FacialDetector:  
    ...
    def get_positive_descriptors(self):
        ...
        images_path = os.path.join(self.params.dir_pos_examples_cropped, '*.jpg')
        ...
        for i in range(num_images):
            ...
            if img.shape[0] < self.params.dim_window or img.shape[1] < self.params.dim_window:
                ...
                new_size = (self.params.dim_window, self.params.dim_window)
    ...
    def run(self):
        ...
        # test_images_path = "./antrenare/validare/validare"
        ...
        for i in range(num_test_images):
            ...
            for resize_factor in resize_factors:
                ...
                num_cols = resized_img.shape[1]  // self.params.dim_hog_cell - 1  #480 / 8  - 1 = 60 - 1 = 59
                num_rows = resized_img.shape[0]  // self.params.dim_hog_cell - 1  #360 / 8  - 1 = 45 - 1 = 44

                num_cell_in_template = self.params.dim_window // self.params.dim_hog_cell - 1 # 64 / 8 - 1 = 7
                # num_cell = window_size // self.params.dim_hog_cell - 1 # 36 / 6 - 1 = 5
                ...
                for y in range(0, num_rows - num_cell_in_template):
                    for x in range(0, num_cols - num_cell_in_template):
                        ...
                        if score > self.params.threshold:
                            x_min = int((x  * self.params.dim_hog_cell) / resize_factor)
                            y_min = int((y  * self.params.dim_hog_cell) / resize_factor)
                            x_max = int((x  * self.params.dim_hog_cell + self.params.dim_window) / resize_factor)
            ...
            if len(image_scores) > 0:
                image_detections, image_scores = self.non_maximal_suppression(np.array(image_detections),
                                                                            np.array(image_scores), resized_img.shape)
            if len(image_scores) > 0:
    ...
    def eval_detections(self, detections, scores, file_names):
        ground_truth_file = np.loadtxt(self.params.path_annotations_test, dtype='str')
